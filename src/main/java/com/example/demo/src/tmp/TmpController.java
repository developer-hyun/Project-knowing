...
@RestController
@RequestMapping("/app/tmp")
public class TmpController {
    final Logger logger = LoggerFactory.getLogger(this.getClass());

    @Autowired
    private final TmpProvider tmpProvider;
    @Autowired
    ...
    public TmpController(TmpProvider tmpProvider, TmpService tmpService, JwtService jwtService) {
        this.tmpProvider = tmpProvider;
...
//    }
    ...
    public BaseResponse<GetMainViewRes> getmain(@RequestHeader(value = "uid") String uid) throws Exception {
        GetMainViewRes getMainViewRes = tmpProvider.getuserinfo(uid);
...
//        return new BaseResponse<>(getUserRes);
    ...
    public BaseResponse<PostUserRes> sociallogin(@PathVariable("uid") String uid,
        ...
        //통신부분
        try {
            PostUserRes postUserRes = tmpService.createsocial(uid, social, postUserReq);
...
//
      ...
      ;
            String status = tmpProvider.kakaologin(email);
            Map<String, Object> additionalClaims = new HashMap<String, Object>();
            additionalClaims.put("id", true);
...
//        }
        ...
        String covidCategory = String.join(" ", postUserSignUpReqTransfer.getMedicalCategory());
        String bookmark = String.join(" ", postUserSignUpReqTransfer.getBookmark());

        if (employmentState.equals("전체")) {       //전체면 이렇게 바꾸기
            employmentState ="미취업자 재직자 프리랜서 일용근로자 단기근로자 예비창업자 자영업자 영농종사자";
        }
...
//
        ...
        try {
            PostUserSignUpRes postUserSingUpRes = tmpService.createmailsignup(uid, postUserSingUpReq);
...
//        }
    ...
    public BaseResponse<GetUserCheckEmail> getUserCheckEmail(@RequestParam("email") String email) throws BaseException {
        try {
            GetUserCheckEmail getUserCheckEmail = new GetUserCheckEmail(tmpProvider.getUsercheckemail(email));
    ...
    public BaseResponse<GetUserCheckEmail> getUsercheckuid(@RequestHeader("uid") String uid) throws BaseException {
        try {
            GetUserCheckEmail getUserCheckEmail = new GetUserCheckEmail(tmpProvider.getUsercheckuid(uid));
            return new BaseResponse<>(getUserCheckEmail);
        } catch (Exception e) {
            return new BaseResponse<>(DATABASE_ERROR);
        }
    ...
    public BaseResponse<GetUserFindId> getUserfindId(@RequestParam("name") String name,
        ...
        GetUserFindId getUserFindId = tmpProvider.getUserFindId(name, phNum);
    ...
    public BaseResponse<GetUserModifyReqTransfer> getUserInfoRes(@RequestHeader("uid") String uid) throws BaseException {
        GetUserModifyReqTransfer getUserModifyReqTransfer = tmpProvider.getUserInfoRes(uid);
    ...
    public BaseResponse<GetUserModifyRes> getUserModify(@RequestHeader("uid") String uid,
        ...
        String covidCategory = String.join(" ", getUserModifyReqTransfer.getMedicalCategory());
        ...
        try {
            GetUserModifyRes getUserModifyRes = tmpService.getUserModifyRes(uid, getUserModifyReq);
    ...
    //    String employmentState = String.join(" ", patchUserWelfareReq.getEmploymentState());
        ...
        String covidCategory = String.join(" ", patchUserWelfareReq.getMedicalCategory());
    //    String bookmark = String.join(" ", patchUserWelfareReq.getBookmark());
        ...
        try {
            GetUserModifyRes getUserModifyRes = tmpService.patchUserWelfareRes(uid, patchUserWelfareStringReq);
            return new BaseResponse<>(getUserModifyRes);
        } catch (BaseException baseException) {
            return new BaseResponse<>((baseException.getStatus()));
        }
    ...
    public BaseResponse<GetUserModifyRes> patchUserPlusInfo(@RequestHeader("userUid") String userUid,
        ...
        try{
            ...
            GetUserModifyRes getUserModifyRes = tmpService.patchUserPlusInfo(userUid,patchUserPlusInfo);
            return new BaseResponse<>(getUserModifyRes);
        } catch (BaseException baseException) {
            return new BaseResponse<>((baseException.getStatus()));
        }
...
//            System.out.println(userRecord);
          //  System.out.println(userProvider.checkProvider(uid));
            if (tmpProvider.checkProvider(uid).equals("default")) {        //default만 파베랑 연동
                UserRecord.UpdateRequest request = new UserRecord.UpdateRequest(uid)
                .setEmail(patchUserPrivacyReq.getEmail());
                UserRecord userRecord = FirebaseAuth.getInstance().updateUser(request);
                System.out.println("Successfully updated user: " + userRecord.getUid()); }

            GetUserModifyRes getUserModifyRes = tmpService.patchUserPrivacy(uid, patchUserPrivacyReq);
            return new BaseResponse<>(getUserModifyRes);
        } catch (BaseException baseException) {
            return new BaseResponse<>((baseException.getStatus()));
        }
...
//rlMaJwc62uTUDA25gxlxrcB82XU2
    ...
    public BaseResponse<DeleteUserRes> deleteUserRes(@RequestHeader("uid") String uid) throws Exception,BaseException {
        try {
            ...
            DeleteUserRes deleteUserRes = tmpService.deleteUserRes(uid);
    ...
    public BaseResponse<PostBookmarkRes> getBookmarkRes(@RequestHeader("userUid") String userUid,
        ...
        PostBookmarkRes postBookmarkRes = tmpProvider.postBookmarkRes(userUid,welfareUid);
    ...
    public BaseResponse<PostBookmarkRes> postBookmarkRes(@RequestHeader("userUid") String userUid,
        ...
        PostBookmarkRes postBookmarkRes = tmpService.postBookmarkRes(userUid,welfareUid);
        return new BaseResponse<>(postBookmarkRes);
    }
    ...
    public BaseResponse<Postfcmplus> fcmplus(@RequestHeader("userUid") String userUid,
        ...
        Postfcmplus postfcmplus = tmpService.fcmplus(userUid, fcmtoken);
